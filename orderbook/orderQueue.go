...
func (oq *OrderQueue) Append(o *Order) *list.Element {
	oq.volume = oq.volume.Add(o.InitialQuantity())
...
func (oq *OrderQueue) Update(e *list.Element, o *Order) *list.Element {
	oq.volume = oq.volume.Sub(e.Value.(*Order).InitialQuantity())
	oq.volume = oq.volume.Add(o.InitialQuantity())
	e.Value = o
	return e
}
...
func (oq *OrderQueue) String() string {
	...
	for iter != nil {
		...
		str := fmt.Sprintf("\n\tid: %s, volume: %s, time: %s", order.OrderId(), order.InitialQuantity(), order.Price())
		sb.WriteString(str)
		iter = iter.Next()
	}
	return sb.String()
